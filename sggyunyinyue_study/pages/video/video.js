...
import request from '../../utils/request'//要发请求就要把request方法引入进来
Page({
   ...
   */
  data: {//下面这些是初始化的数据
    videoGroupList: [],//导航标签数据。发请求要拿导航标签数据，所以要初始化一个数组
    navId: '',//导航的标识
    videoList: [],//视频列表数据
    videoId: '',//视频id标识
  ...
  onLoad: function (options) {
    //获取导航数据
    this.getVideoGroupListData();// 调用下面的方法
  },
  ...
  async getVideoGroupListData(){
    ...
    // 拿到数据后更新
    this.setData({
      // 要更新的对象是videoGroupList，数据源是videoGroupListData，最外层的对象是data，截取0-14个导航数据
      videoGroupList: videoGroupListData.data.slice(0,14),
      navId: videoGroupListData.data[0].id//作用：在刚加载页面的时候就显示下边框
    })
    // console.log(this.data.navId,'111');

    //获取视频列表数据
    // console.log(this.data.navId,'222');
  ...
  async getVideoList(navId){
    if(!navId){//判断navId为空串的情况
      return;//如果为false就不让它发请求。有值就不是空串了，就为true了
    }
    // http://localhost:3000/video/group?id=58100
    let videoListData = await request('/video/group',{id: navId});
    // 关闭消息提示框
    wx.hideLoading();//在拿到数据后就关闭消息提示框
    
    console.log(videoListData);//看是否拿到数据
    let index = 0;
  ...
  //点击切换导航的回调
  changeNav(event){
    let navId = event.currentTarget.id;//拿到id标识// 通过id向event传参的时候如果传的是number会自动转换成string
    // console.log(typeof navId);//打印navId的类型
    this.setData({
      navId: navId*1,//将标识更新到data中
      // navId: navId>>>0//通过位移的方式也可以转
      videoList: []
    })
    ...
    // 动态获取当前导航对应的视频数据
    this.getVideoList(this.data.navId);//传入当前最新的navId
  },
  ...
  handlePlay(event){
    ...
    * */
    ...
    // 写法二：
    // if(this.vid !== vid){//意思是如果this.vid !== vid为true//要判断当前播放的视频与之前播放的视频不是同一个视频才停止
    //   if(this.videoContext){//意思是如果(this.videoContext为true
  ...
  handleTimeUpdate(event){
    ...
    * */
    ...
    if(videoItem){ // 之前有
      ...
      videoUpdateTime.push(videoTimeObj);//如果没有if-else判断，直接写这行代码，所有的视频vid都会一样
    }
  ...
  handleEnded(event){
    // console.log('播放结束');
    // 移除记录播放时长数组中当前视频的对象
    let {videoUpdateTime} = this.data;//把数组拿出来
    videoUpdateTime.splice(videoUpdateTime.findIndex(item => item.vid === event.currentTarget.id), 1);//1表示移除1个。找数组下标通过findIndex去找
    this.setData({
      videoUpdateTime
    })
  },
  ...
  handleToLower(){
    ...
    // 数据分页： 1. 后端分页(你发一个请求我只给10条)由后端控制我给你几次，即分割数据在后端发生。后端分页的特点是：我们发请求需要携带指定的参数，比如哪一页的数据 2. 前端分页(假设有100条数据，一次只给用户显示10条)前端分页是后端会一次把100条数据给前端，前端拿到后一次显示10条
    console.log('发送请求 || 在前端截取最新的数据 追加到视频列表的后方');
    console.log('网易云音乐暂时没有提供分页的api');
    // 这里没办法进行前端分页和后端分页，因为网易云音乐没有提供api.所以下面进行模拟，把数据追加到后面。注意：这个模拟只是把旧的数据追加到后面

    // 模拟数据
    // 数据从http://localhost:3000/video/group?id=59101这里面拿
    let newVideoList = [
      {
        "type": 1,
        "displayed": false,
    ...
    // 将视频最新的数据更新原有视频列表数据中
    videoList.push(...newVideoList);//用三点运算符把数组拆包，然后放到视频的数组中
    this.setData({
      videoList
    })
  },
